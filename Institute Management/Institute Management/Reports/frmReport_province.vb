...
Public Class frmReport_province
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Dim adapter As New MySqlDataAdapter
        'Dim ds As New DataSet1
        Dim ds As New DS_Province

        If MysqlConn.State = ConnectionState.Closed Then
            MysqlConn.Open()
        End If

        adapter.SelectCommand = New MySqlCommand("SELECT province_id,province_kh,province_en FROM tbl_province", MysqlConn)
        adapter.Fill(ds.Tables(0))
        rptProvince.ProcessingMode = ProcessingMode.Local
        ...
        rptProvince.LocalReport.ReportPath = path1 & ("\rptProvince.rdlc")

        'ReportViewer1.LocalReport.ReportPath = System.Environment.CurrentDirectory & ("\Report1.rdlc")
        rptProvince.LocalReport.DataSources.Clear()

        rptProvince.LocalReport.DataSources.Add(New Microsoft.Reporting.WinForms.ReportDataSource("DS_ProvinceReport", ds.Tables(0)))


        rptProvince.DocumentMapCollapsed = True
        Me.rptProvince.RefreshReport()
    End Sub
End Class